...
def confidence_filter(
    ...
    *,
    ...
    max_ce = float(scores[0])
    band_threshold = max_ce - float(delta)

    K_clean = 0
    for i in range(M):
        if scores[i] >= band_threshold and scores[i] >= A_tail:
            K_clean += 1
        else:
            break
    ...
    if K_final < MIN_RETURN:
        ...
        if forced > K_final:
            print(
                ...
                f"(best={scores[0]:.3f}, A_best={A_best}, A_tail={A_tail})"
            )
        K_final = forced
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--sort_by_announced_date",
        action="store_true",
        help="Sort CE results by announced_date before confidence filtering",
    )

    # Confidence filter (design flags)
    p.add_argument("--a_best", type=float, default=0.0, help="Viability threshold for top ce_score")
    p.add_argument("--a_tail", type=float, default=-3.5, help="Tail floor threshold for ce_score")
    p.add_argument("--delta", type=float, default=4.5, help="Band delta from max ce_score")
