...
Start the search backend (from `iptp_search`):

```bash
python -m scripts.api --port 8010
```

Start the UI (from `iptp_ui`):

```bash
python3 -m http.server 4173
```

Then open: `http://localhost:4173/search.html?query=asylum+policy`

The UI calls the backend at `http://127.0.0.1:8010`. Both must be running for search to work.
